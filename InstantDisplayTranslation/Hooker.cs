...
namespace FLib
{
    ...
    public class Hooker
    {
        ...
        public Func<int, WM, IntPtr, Hooker, bool> OnMouseHook = null;
        ...
        int OnMouseLLHook(int code, WM message, IntPtr state)
        {
            try
            {
                bool ignore = false;
                if (OnMouseHook != null)
                    ignore = OnMouseHook(code, message, state, this);
                return ignore ? 1 : CallNextHookEx(mouseHook, code, message, state);
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
